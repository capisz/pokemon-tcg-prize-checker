export type FeaturedDeckDefinition = {
  id: string
  title: string
  sourceUrl: string
  player: string
  playedBy: string
  importText: string
}

export const FEATURED_DECKS: FeaturedDeckDefinition[] = [
  {
    id: "27608",
    title: "Alakazam Dudunsparce",
    sourceUrl: "https://limitlesstcg.com/decks/list/27608",
    player: "Cerys Jones",
    playedBy: "1st Place Regional Indianapolis, IN - Cerys Jones",
    importText: `Pokemon: 22
4 Abra MEG 54
4 Kadabra MEG 55
3 Alakazam MEG 56
3 Dunsparce JTG 120
3 Dudunsparce TEF 129
1 Fezandipiti ex ASC 142
1 Dedenne SSP 87
1 Elgyem BLK 40
1 Genesect SFA 40
1 Psyduck ASC 39

Trainer: 32
4 Dawn PFL 87
3 Hilda WHT 84
2 Boss's Orders MEG 114
1 Lana's Aid TWM 155
4 Buddy-Buddy Poffin TEF 144
4 Poké Pad POR 81
3 Rare Candy MEG 125
2 Enhanced Hammer TWM 148
1 Sacred Ash DRI 168
1 Night Stretcher ASC 196
2 Handheld Fan TWM 150
1 Lucky Helmet TWM 158
4 Nighttime Mine ASC 197

Energy: 6
4 Telepathic Psychic Energy POR 88
1 Psychic Energy MEE 5
1 Enriching Energy SSP 191`,
  },
  {
    id: "27609",
    title: "Crustle",
    sourceUrl: "https://limitlesstcg.com/decks/list/27609",
    player: "Rahul Reddy",
    playedBy: "2nd Place Regional Indianapolis, IN - Rahul Reddy",
    importText: `Pokemon: 12
4 Mega Kangaskhan ex MEG 104
3 Dwebble DRI 11
3 Crustle DRI 12
1 Cornerstone Mask Ogerpon ex TWM 112
1 Psyduck ASC 39

Trainer: 35
4 Lillie's Determination MEG 119
4 Boss's Orders MEG 114
4 Team Rocket's Petrel DRI 176
2 Hilda WHT 84
1 Eri TEF 146
1 Xerosic's Machinations SFA 64
1 Pokémon Center Lady MEG 123
1 Bianca's Devotion TEF 142
4 Crushing Hammer POR 71
4 Jumbo Ice Cream PFL 91
3 Pokégear 3.0 SVI 186
2 Buddy-Buddy Poffin TEF 144
1 Ultra Ball MEG 131
1 Switch MEG 130
1 Hero's Cape TEF 152
1 Team Rocket's Factory DRI 173

Energy: 13
4 Spiky Energy JTG 159
4 Growing Grass Energy POR 86
4 Mist Energy TEF 161
1 Fighting Energy MEE 6`,
  },
  {
    id: "27610",
    title: "Dragapult Dusknoir",
    sourceUrl: "https://limitlesstcg.com/decks/list/27610",
    player: "Justin Newdorf",
    playedBy: "3rd Place Regional Indianapolis, IN - Justin Newdorf",
    importText: `Pokemon: 21
4 Dreepy TWM 128
4 Drakloak TWM 129
3 Dragapult ex TWM 130
2 Duskull PRE 35
2 Dusclops PRE 36
1 Dusknoir PRE 37
1 Fezandipiti ex ASC 142
1 Munkidori TWM 95
1 Budew ASC 16
1 Meowth ex POR 62
1 Moltres PFL 14

Trainer: 31
4 Lillie's Determination MEG 119
3 Boss's Orders MEG 114
3 Crispin SCR 133
1 Dawn PFL 87
4 Buddy-Buddy Poffin TEF 144
4 Poké Pad POR 81
4 Ultra Ball MEG 131
3 Crushing Hammer POR 71
2 Night Stretcher ASC 196
1 Unfair Stamp TWM 165
2 Team Rocket's Watchtower DRI 180

Energy: 8
4 Fire Energy MEE 2
3 Psychic Energy MEE 5
1 Darkness Energy MEE 7`,
  },
  {
    id: "27614",
    title: "Hydrapple",
    sourceUrl: "https://limitlesstcg.com/decks/list/27614",
    player: "Cody Hope",
    playedBy: "10th Place Regional Indianapolis, IN - Cody Hope",
    importText: `Pokemon: 21
4 Teal Mask Ogerpon ex TWM 25
2 Chikorita MEG 8
2 Bayleef MEG 9
2 Meganium MEG 10
2 Applin TWM 17
2 Dipplin TWM 18
2 Hydrapple ex SCR 14
2 Meowth ex POR 62
1 Celebi MEG 12
1 Tapu Bulu SFA 6
1 Fezandipiti ex ASC 142

Trainer: 25
4 Lillie's Determination MEG 119
2 Boss's Orders MEG 114
2 Dawn PFL 87
1 Briar SCR 132
1 Lana's Aid TWM 155
1 Ciphermaniac's Codebreaking TEF 145
4 Ultra Ball MEG 131
4 Bug Catching Set TWM 143
1 Unfair Stamp TWM 165
1 Night Stretcher ASC 196
4 Forest of Vitality MEG 117

Energy: 14
14 Grass Energy MEE 1`,
  },
  {
    id: "28249",
    title: "Lillie's Clefairy",
    sourceUrl: "https://limitlesstcg.com/decks/list/28249",
    player: "James Kowalski",
    playedBy: "1st Place NAIC 2026, New Orleans - James Kowalski",
    importText: `Pokemon: 22
4 Mega Kangaskhan ex MEG 104
4 Meowth ex POR 62
4 Lillie's Clefairy ex JTG 56
3 Latias ex SSP 76
2 Wellspring Mask Ogerpon ex TWM 64
2 Fezandipiti ex ASC 142
1 Moltres PFL 14
1 Chien-Pao SSP 56
1 Koraidon ex ASC 121

Trainer: 28
4 Crispin SCR 133
3 Boss's Orders MEG 114
2 Ciphermaniac's Codebreaking TEF 145
1 Cyrano SSP 170
4 Ultra Ball MEG 131
4 Dusk Ball SSP 175
3 Wondrous Patch PFL 94
1 Prime Catcher TEF 157
2 Lillie's Pearl JTG 151
4 Area Zero Underdepths SCR 131

Energy: 10
4 Psychic Energy MEE 5
2 Water Energy MEE 3
2 Fighting Energy MEE 6
1 Telepathic Psychic Energy POR 88
1 Fire Energy MEE 2`,
  },
]
