...
export function PracticeModePicker({value,onChange}:{value:number;onChange:(value:number)=>void}){
 ...
 <button ref={trigger} type="button" aria-label="Practice mode" aria-haspopup="listbox" aria-expanded={open} onClick={()=>setOpen(!open)} onKeyDown={event=>{if(['ArrowDown','ArrowUp'].includes(event.key)){event.preventDefault();setOpen(true)}}} className="flex h-9 w-full items-center justify-between gap-2 rounded-full border border-emerald-500/35 bg-emerald-900/80 px-4 text-xs font-semibold text-emerald-200 outline-none hover:bg-emerald-900 focus-visible:ring-2 focus-visible:ring-emerald-400/50">
